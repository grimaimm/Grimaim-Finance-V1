...
import os, re
from flask import Blueprint, Flask, flash, render_template, request, url_for, redirect, session, jsonify
...
from models.get_total import get_unique_transaction_dates, get_total_pemasukan_by_date, get_total_pengeluaran_by_date, get_total_pengeluaran_by_kategori_semua, get_total_pengeluaran_by_kategori_harian, get_total_pengeluaran_by_kategori_mingguan, get_total_pengeluaran_by_kategori_bulanan
from models.dashboard import get_total_pengeluaran_pemasukan_hari_ini, get_total_pengeluaran_pemasukan_mingguan, get_total_pengeluaran_pemasukan_bulanan, get_total_pengeluaran_pemasukan_tahunan, get_total_transaksi, get_total_transaksi_by_admin, get_monthly_data
...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + os.path.join(basedir, "grimaimDash.db")
...
def register():
    ...
    if request.method == "POST":
        ...
        if existing_user:
            ...
            new_user = User(username=username, fullname=fullname, password=password, userPicture='default-image.jpg')
...
def dashboard():
    ...
    today = date.today()
    total_pengeluaran_hari_ini, total_pemasukan_hari_ini = get_total_pengeluaran_pemasukan_hari_ini(user_id, today)

    # Hitung total pengeluaran dan pemasukan kemarin
    yesterday = today - timedelta(days=1)
    total_pengeluaran_kemarin, total_pemasukan_kemarin = get_total_pengeluaran_pemasukan_hari_ini(user_id, yesterday)

    # Mengambil data total keseluruhan transaksi
    total_transaksi_pemasukan, total_transaksi_pengeluaran = get_total_transaksi(user_id)

    # Mengambil data pengeluaran dan pemasukan dari admin input
    admin_data = ["Aim", "Dhian"]
    admin_stats = []
    for admin_name in admin_data:
        total_pengeluaran, total_pemasukan = get_total_transaksi_by_admin(user_id, admin_name)
        admin_stats.append({
...
def dashboardMingguan():
    ...
    total_pengeluaran_minggu_ini, total_pemasukan_minggu_ini = get_total_pengeluaran_pemasukan_mingguan(user_id, start_of_week, end_of_week)
    ...
    total_pengeluaran_minggu_lalu, total_pemasukan_minggu_lalu = get_total_pengeluaran_pemasukan_mingguan(user_id, start_of_previous_week, end_of_previous_week)
...
def dashboardBulanan():
    ...
    total_pengeluaran_bulan_ini, total_pemasukan_bulan_ini, total_pengeluaran_bulan_lalu, total_pemasukan_bulan_lalu = get_total_pengeluaran_pemasukan_bulanan(user_id, current_month)
...
def dashboardTahunan():
    ...
    total_pengeluaran_tahun_ini, total_pemasukan_tahun_ini = get_total_pengeluaran_pemasukan_tahunan(user_id, current_year)
    total_pengeluaran_tahun_lalu, total_pemasukan_tahun_lalu = get_total_pengeluaran_pemasukan_tahunan(user_id, last_year)
...
def pengeluaran():
    ...
    semuaPengeluaran = Pengeluaran.query.filter_by(userID=current_user.userID).order_by(
        Pengeluaran.tanggalPengeluaran.desc() if sort_order == 'desc' else Pengeluaran.tanggalPengeluaran.asc()
    )
...
def pengeluaranHarian():
    ...
    today = date.today()
    semuaPengeluaran_Harian = Pengeluaran.query.filter_by(userID=current_user.userID, tanggalPengeluaran=today).all()

    # Urutkan berdasarkan urutan yang dipilih = ascending
    reverse_sort = sort_order == 'asc'
    semuaPengeluaran_Harian_sorted = sorted(semuaPengeluaran_Harian, key=lambda x: x.tanggalPengeluaran, reverse=reverse_sort)
...
def pengeluaranMingguan():
    ...
    # Urutkan berdasarkan urutan yang dipilih = ascending
    reverse_sort = sort_order == 'asc'
    semuaPengeluaran_Mingguan_sorted = sorted(semuaPengeluaran_Mingguan, key=lambda x: x.tanggalPengeluaran, reverse=reverse_sort)
...
def pengeluaranBulanan():
    ...
    # Urutkan berdasarkan urutan yang dipilih = ascending 
    reverse_sort = sort_order == 'asc'
    semuaPengeluaran_Bulanan_sorted = sorted(semuaPengeluaran_Bulanan, key=lambda x: x.tanggalPengeluaran, reverse=reverse_sort)
...
def tambahPengeluaran():
    ...
    if request.method == "POST":
        ...
        tanggalPengeluaran = datetime.strptime(tanggalPengeluaran_str, '%Y-%m-%d').date()
        ...
        jumlahPengeluaran = float(request.form.get("jumlahPengeluaran").replace('Rp. ', '').replace('.', ''))
        ...
        return render_template('pengeluaran/tambahPengeluaran.html', 
                                userInfo=userInfo, 
...
def editPengeluaran(pengeluaranID):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(f'Data pengeluaran dengan ID: { pengeluaranID } berhasil diupdate', 'success')
        return redirect(url_for('pengeluaran'))
        ...
        return render_template('pengeluaran/editPengeluaran.html',
                                userInfo=userInfo,
                                kategori_list=kategori_list,
                                admin_list=admin_list,
...
def deletePengeluaran(pengeluaranID):
    ...
    if pengeluaran:
        ...
        db.session.commit()
        flash(f'Data pengeluaran dengan ID: { pengeluaranID } berhasil dihapus', 'success')
    else:
...
def pemasukan():
    ...
    semuaPemasukan = Pemasukan.query.filter_by(userID=current_user.userID).order_by(
        Pemasukan.tanggalPemasukan.desc() if sort_order == 'desc' else Pemasukan.tanggalPemasukan.asc()
    )
...
def pemasukanHarian():
    ...
    today = date.today()
    semuaPemasukan_Harian = Pemasukan.query.filter_by(userID=current_user.userID, tanggalPemasukan=today).all()

    # Urutkan data pemasukan harian sesuai parameter sort_order
    reverse_sort = sort_order == 'asc'
    semuaPemasukan_Harian_sorted = sorted(semuaPemasukan_Harian, key=lambda x: x.tanggalPemasukan, reverse=reverse_sort)
...
def pemasukanMingguan():
    ...
    reverse_sort = sort_order == 'asc'
    semuaPemasukan_Mingguan_sorted = sorted(semuaPemasukan_Mingguan, key=lambda x: x.tanggalPemasukan, reverse=reverse_sort)
...
def pemasukanBulanan():
    ...
    reverse_sort = sort_order == 'asc'
    semuaPemasukan_Bulanan_sorted = sorted(semuaPemasukan_Bulanan, key=lambda x: x.tanggalPemasukan, reverse=reverse_sort)
...
def tambahPemasukan():
    ...
    if request.method == "POST":
        ...
        jumlahPemasukan = float(request.form.get("jumlahPemasukan").replace('Rp. ', '').replace('.', ''))
        adminName = request.form.get('adminName')
        ...
        return render_template('pemasukan/tambahPemasukan.html', 
                                userInfo=userInfo, 
                                kategori_list=kategori_list, 
                                admin_list=admin_list, 
                                current_date=current_date)
...
def editPemasukan(pemasukanID):
    ...
    if request.method == "POST":
        ...
        return render_template('pemasukan/editPemasukan.html',
                                userInfo=userInfo,
                                kategori_list=kategori_list,
                                admin_list=admin_list,
                                pemasukan=pemasukan,
...
def keuangan():
    ...
    unique_dates = set(item.tanggalPemasukan for item in semuaPemasukan) | set(item.tanggalPengeluaran for item in semuaPengeluaran)
...
def keuanganHarian():
    ...
    # Query semua pemasukan harian pengguna untuk hari ini
    today = date.today()
    semuaPemasukan_Harian = Pemasukan.query.filter_by(userID=current_user.userID, tanggalPemasukan=today).order_by(
        Pemasukan.tanggalPemasukan.desc() if sort_order == 'desc' else Pemasukan.tanggalPemasukan.asc()
    ).all()

    # Query semua pengeluaran harian pengguna untuk hari ini
    semuaPengeluaran_Harian = Pengeluaran.query.filter_by(userID=current_user.userID, tanggalPengeluaran=today).order_by(
        Pengeluaran.tanggalPengeluaran.desc() if sort_order == 'desc' else Pengeluaran.tanggalPengeluaran.asc()
    ).all()

    # Menggabungkan tanggal unik dari pemasukan dan pengeluaran
    unique_dates = set(item.tanggalPemasukan for item in semuaPemasukan_Harian) | set(item.tanggalPengeluaran for item in semuaPengeluaran_Harian)
...
def keuanganMingguan():
    ...
    # Menggabungkan tanggal unik dari pemasukan dan pengeluaran
    unique_dates = set(item.tanggalPemasukan for item in semuaPemasukan_Mingguan) | set(item.tanggalPengeluaran for item in semuaPengeluaran_Mingguan)
...
def keuanganBulanan():
    ...
    # Menggabungkan tanggal unik dari pemasukan dan pengeluaran
    unique_dates = set(item.tanggalPemasukan for item in semuaPemasukan_Bulanan) | set(item.tanggalPengeluaran for item in semuaPengeluaran_Bulanan)
...
def kategori():
    ...
    total_pengeluaran_by_kategori_semua = get_total_pengeluaran_by_kategori_semua(user_id)
...
def kategori_harian():
    ...
    total_pengeluaran_by_kategori_harian = get_total_pengeluaran_by_kategori_harian(user_id, date=today)
    total_pengeluaran_yesterday = get_total_pengeluaran_by_kategori_harian(user_id, date=yesterday)
...
def kategori_mingguan():
    ...
    total_pengeluaran_by_kategori_mingguan = get_total_pengeluaran_by_kategori_mingguan(user_id, start_of_week, end_of_week)
    ...
    total_pengeluaran_previous_week = get_total_pengeluaran_by_kategori_mingguan(user_id, start_of_previous_week, end_of_previous_week)
...
def kategori_bulanan():
    ...
    total_pengeluaran_by_kategori_bulanan = get_total_pengeluaran_by_kategori_bulanan(user_id, start_of_month, end_of_month)
    ...
    total_pengeluaran_previous_month = get_total_pengeluaran_by_kategori_bulanan(user_id, start_of_previous_month, end_of_previous_month)
...
def account(username):
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if existing_user and new_username != username:
                flash("Akun dengan username tersebut sudah ada. Gunakan username lain.", "error")
    ...
    return render_template('users/account.html', userInfo=userInfo, timestamp=timestamp, active_tab=active_tab)
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if new_password and new_password != user.password:
                ...
                flash("Kata sandi baru harus berbeda dengan kata sandi saat ini", "error")
...
def delete_account(username):
    ...
    return render_template('users/account.html', userInfo=userInfo, active_tab=active_tab)
